...
def get_orders():
    ...
    product_ids = list(set(it.product_id for it in all_items))
    products_dict = {p.id: p for p in Product.query.filter(Product.id.in_(product_ids)).all()} if product_ids else {}
    
    # Group items by order
    items_by_order = {}
    for it in all_items:
        p = products_dict.get(it.product_id)
        mapped = {
        ...
        items_by_order.setdefault(it.order_id, []).append(mapped)

    for o in orders:
        cust = users_dict.get(o.user_id)
        mapped_items = items_by_order.get(o.id, [])
        results.append({
            "id": o.id,
